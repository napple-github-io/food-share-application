...
module.exports = Router()
  ...
  .get('/nearme', ensureAuth, (req, res, next) => {
    Listing
      .find({ archived: false })
      .then(list => {
        list.map(async listing => {
          const distance = await getDistanceByAddress(req.user.location, listing.location);
          if(distance < 10)
            return { distance, id: listing._id };
        });
      })
      .then(listings => {
        return Promise.all([
          Listing.findById(listings.id)
        ]);
      })
