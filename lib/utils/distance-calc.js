...
function getDistanceByZip(originZip, destZip) {
  const url = `https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=${originZip}&destinations=${destZip}&key=${process.env.API_KEY}`;
  return request
    .get(url)
    .then(res => res.body)
    .then(result => result.rows[0].elements[0].distance.text)
